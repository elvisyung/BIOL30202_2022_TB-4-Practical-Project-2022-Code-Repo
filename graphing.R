...
# ------------------------------------------------
# Statistical Analyse for Eukaryote Table (Alpha Only)
df_eukaryote_alpha <- read.table('~/Desktop/stats/eukaryote_alpha.csv', sep =",",header = T)

# Checking for Normal Distribution (Assumptions)
model_eukaryote_alpha <- lm(genus_values ~ treatment, data = df_eukaryote_alpha)
residuals_eukaryote_alpha <- residuals(model_eukaryote_alpha)
hist(residuals_eukaryote_alpha)
qqnorm(residuals_eukaryote_alpha)
qqline(residuals_eukaryote_alpha, col = "blue")
fitted_eukaryote_alpha <- fitted(model_eukaryote_alpha)
plot(residuals_eukaryote_alpha ~ fitted_eukaryote_alpha)

# SQRT, does not work 
SQRT_model_combined <-lm(genus_values ~ treatment, data = df_eukaryote_alpha)
SQRT_residuals <- residuals(SQRT_model_combined)
hist(SQRT_residuals)
qqnorm(SQRT_residuals)
qqline(SQRT_residuals, col = "blue")
...
# Power, does not work 
power_combined_model <- lm((genus_values^2) ~ treatment, data = df_eukaryote_alpha)
power_residuals <- residuals(power_combined_model)
hist(power_residuals)
qqnorm(power_residuals)
qqline(power_residuals)
...
# Reciprocal, does not work 
reciprocal_model <- lm((1/(genus_values)) ~ treatment, data = df_eukaryote_alpha)
reciprocal_resdiuals <- residuals(reciprocal_model)
hist(reciprocal_resdiuals)
qqnorm(reciprocal_resdiuals)
qqline(reciprocal_resdiuals)
...
# Log, does not work
log_model <- lm(log(genus_values) ~ treatment, data = df_eukaryote_alpha)
log_resdiuals <- residuals(log_model)
hist(log_resdiuals)
qqnorm(log_resdiuals)
qqline(log_resdiuals)
...
# Not Normal after data transformation as well (Mann-Whitney U Test)
wilcox.test(genus_values ~ treatment, data = df_eukaryote_alpha)
...
# Statistical Analyse for Eukaryote Table (Proportions)
df_eukaryote_prop <- read.table('~/Desktop/stats/eukaryote_prop.csv', sep =",",header = T)

# Checking for Normal Distribution (Assumptions)
model_eukaryote_prop <- lm(proportion_values ~ treatment, data = df_eukaryote_prop)
residuals_eukaryote_prop <- residuals(model_eukaryote_prop)
hist(residuals_eukaryote_prop)
qqnorm(residuals_eukaryote_prop)
qqline(residuals_eukaryote_prop, col = "blue")
fitted_eukaryote_prop <- fitted(model_eukaryote_prop)
plot(residuals_eukaryote_prop ~ fitted_eukaryote_prop)

kruskal.test(proportion_values ~ treatment, data = df_eukaryote_prop)
